...
import { rewriteSectionAssets } from '../epubAssets';
...
function prefetchSpine(book: Book, spineIndex: number): void {
  ...
  void Promise.all(
    keep.map((index) => {
      ...
      return readEpubEntryCached(normalizeEpubEntryPath(url)).catch(() => null);
    }),
  );
}
